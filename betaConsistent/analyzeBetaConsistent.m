...
betaConsistent = load('../../20c-RidgeRegression_arm/betaConsistent/betaConsistent.txt');
betaConsistentNoCoulomb = betaConsistent; 
betaConsistentNoCoulomb(13:13:91) = 0;
...
%%
trainCurrPredictConsistent = transpose(reshape(trainPhi*betaConsistent,[7,length(trainPhi)/7]))/diag(km)/diag(G_R);
testCurrPredictConsistent = transpose(reshape(testPhi*betaConsistent,[7,length(testPhi)/7]))/diag(km)/diag(G_R);
trainCurrPredictConsistentNoCoulomb = transpose(reshape(trainPhi*betaConsistentNoCoulomb,[7,length(trainPhi)/7]))/diag(km)/diag(G_R);
testCurrPredictConsistentNoCoulomb = transpose(reshape(testPhi*betaConsistentNoCoulomb,[7,length(testPhi)/7]))/diag(km)/diag(G_R);
...
for i=1:7
   ...
   plot(trainTime, trainCurr(2:end, i), ...
        trainTime, trainCurrPredictConsistent(:, i), ...
        trainTime, trainCurrPredictConsistentNoCoulomb(:, i), ...
        'LineWidth', 2);
   xlabel('time (s)');
   if(i==7)
        legend({'q','$$\dot{q}$$', '$$\ddot{q}$$', '$$i_{measured}$$', ...
            '$$i_{predicted} (\beta_{consistent})$$',...
            '$$i_{predicted} (\beta_{consistent}, \Gamma_{couloumb} = 0)$$' ...
...
for i=1:7
   ...
   plot(testTime, testCurr(2:end, i), ...
        testTime, testCurrPredictConsistent(:, i), ...
        testTime, testCurrPredictConsistentNoCoulomb(:, i), ...
        'LineWidth', 2);
   xlabel('time (s)');
   if(i==7)
        legend({'q','$$\dot{q}$$', '$$\ddot{q}$$', '$$i_{measured}$$', ...
